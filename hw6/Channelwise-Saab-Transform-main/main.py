...
if __name__ == "__main__":
    ...
    p2_base = Pixelhop(depth=3, TH1=thresh1, TH2=thresh2, 
    ...
    print('Getting the hop3 features for PixelHop')
    train_hop3_feats_base = get_feat(x_train_reduced, p2_base)
    ...
    train_plus_reshaped = np.reshape(train_hop3_feats_plus, (N_train_plus, X_train_plus))
    test_plus_reshaped = np.reshape(test_hop3_feats_plus, (N_test_plus, X_test_plus))
    print(train_plus_reshaped.shape)
    print(test_plus_reshaped.shape)

    N_train_base, _, _, X_train_base = train_hop3_feats_base.shape
    N_test_base, _, _, X_test_base = test_hop3_feats_base.shape
    train_base_reshaped = np.reshape(train_hop3_feats_base, (N_train_base, X_train_base))
    test_base_reshaped = np.reshape(test_hop3_feats_base, (N_test_base, X_test_base))
    ...
    print('Fitting xgboost on PixelHop')
    clf_base.fit(train_base_reshaped, y_train_reduced)
    ...
    disp_base = ConfusionMatrixDisplay(confusion_matrix=cm_base, display_labels=fashion_labels)
    disp_base.plot()
    plt.show()
