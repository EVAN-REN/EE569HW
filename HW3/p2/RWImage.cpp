...
void RWImage::writeRawImage(std::string path, std::vector<std::vector<std::vector<unsigned char> > > inputImage, int BytesPerPixel){
    ...
    unsigned char Imagedata[height * 2][width * 3][BytesPerPixel];
    ...
    fwrite(Imagedata, sizeof(unsigned char), 6 * width * height * BytesPerPixel, file);
...
cv::Mat RWImage::transformToMat(std::vector<std::vector<std::vector<unsigned char> > > image){
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            result.at<cv::Vec3b>(i, j)[0] = image[i][j][2];
            result.at<cv::Vec3b>(i, j)[1] = image[i][j][1]; 
            result.at<cv::Vec3b>(i, j)[2] = image[i][j][0]; 
        }
    }
