...
ColorImage::ColorImage(int h, int w, int b):height(h), width(w), BytesPerPixel(b){

}

// Read image (filename specified by first argument) into image data matrix
void ColorImage::readRawImage(std::string path){
    FILE *file;
...
void ColorImage::writeRawImage(std::string path, int type, std::vector<std::vector<std::vector<unsigned char> > > wImage){
    FILE *file;
    ...
    // calculate psnr
    std::cout << "ColorImage_psnr_R:" << calculatePSNR(wImage, 0) << std::endl;
    std::cout << "ColorImage_psnr_G:" << calculatePSNR(wImage, 1) << std::endl;
    std::cout << "ColorImage_psnr_B:" << calculatePSNR(wImage, 2) << std::endl;
...
std::vector<std::vector<std::vector<unsigned char> > > ColorImage::meanFilter(std::vector<std::vector<std::vector<unsigned char> > > inputImage){
    ...
    for(int i = 0; i < height; i++){
        for(int j = 0; j < width; j++){
            if(i == 0 || i == height - 1 || j == 0 || j == width - 1){
                ...
                result[i][j][0] = (inputImage[i - 1][j - 1][0] + inputImage[i][j - 1][0] + inputImage[i + 1][j - 1][0] + inputImage[i - 1][j][0] + inputImage[i][j][0] + inputImage[i + 1][j][0] + inputImage[i - 1][j + 1][0] + inputImage[i][j + 1][0] + inputImage[i + 1][j + 1][0]) / 9; 
...
std::vector<std::vector<std::vector<unsigned char> > >  ColorImage::GaussianFilter(std::vector<std::vector<std::vector<unsigned char> > > inputImage, int lenSide, double sigma){
    std::vector<std::vector<std::vector<unsigned char>>> result(height, std::vector<std::vector<unsigned char>>(width, std::vector<unsigned char>(3, 0)));
...
std::vector<std::vector<std::vector<unsigned char> > >  ColorImage::bilateralFilter(std::vector<std::vector<std::vector<unsigned char> > > inputImage, int lenSide, double sigmac, double sigmas){
    std::vector<std::vector<std::vector<unsigned char>>> result(height, std::vector<std::vector<unsigned char>>(width, std::vector<unsigned char>(3, 0)));
...
std::vector<std::vector<std::vector<unsigned char> > > ColorImage::NLMFilter(std::vector<std::vector<std::vector<unsigned char> > > inputImage, float intensity, int templateWindowSize, int searchWindowSize){
    std::vector<std::vector<std::vector<unsigned char>>> result(height, std::vector<std::vector<unsigned char>>(width, std::vector<unsigned char>(3, 0)));
    cv::Mat noisyImage(height, width, CV_8U);

    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
            noisyImage.at<unsigned char>(i, j) = inputImage[i][j][0];
        }
    }
...
std::vector<std::vector<std::vector<unsigned char> > > ColorImage::RGB2YUV(){
    std::vector<std::vector<std::vector<unsigned char>>> result(height, std::vector<std::vector<unsigned char>>(width, std::vector<unsigned char>(3, 0)));
    for(int i = 0; i < height; i++){
        for(int j = 0; j < width; j++){
...
std::vector<std::vector<std::vector<unsigned char> > > ColorImage::YUV2RGB(std::vector<std::vector<std::vector<unsigned char> > > inputImage){
    std::vector<std::vector<std::vector<unsigned char>>> result(height, std::vector<std::vector<unsigned char>>(width, std::vector<unsigned char>(3, 0)));
    for(int i = 0; i < height; i++){
        for(int j = 0; j < width; j++){
...
std::vector<std::vector<std::vector<unsigned char> > >  ColorImage::medianFilter(std::vector<std::vector<std::vector<unsigned char> > > inputImage, int windowSize){
    std::vector<std::vector<std::vector<unsigned char>>> result(height, std::vector<std::vector<unsigned char>>(width, std::vector<unsigned char>(3, 0)));
...
std::vector<std::vector<std::vector<unsigned char> > > ColorImage::RGB2HSV(){
    std::vector<std::vector<std::vector<unsigned char>>> result(height, std::vector<std::vector<unsigned char>>(width, std::vector<unsigned char>(3, 0)));
    ...
    for(int i = 0; i < height; i++){
        for(int j = 0; j < width; j++){
            for(int k = 0; k < 3; k++){
                mat.at<cv::Vec3b>(i, j)[k] = image[i][j][k];
            }
        }
    }
    ...
    for(int i = 0; i < height; i++){
        for(int j = 0; j < width; j++){
            for(int k = 0; k < 3; k++){
                result[i][j][k] = mat.at<cv::Vec3b>(i, j)[k];
            }
        }
    }
...
std::vector<std::vector<std::vector<unsigned char> > > ColorImage::HSV2RGB(std::vector<std::vector<std::vector<unsigned char> > > inputImage){
    std::vector<std::vector<std::vector<unsigned char>>> result(height, std::vector<std::vector<unsigned char>>(width, std::vector<unsigned char>(3, 0)));
    cv::Mat mat(height, width, CV_8UC3);
    ...
    for(int i = 0; i < height; i++){
        for(int j = 0; j < width; j++){
            for(int k = 0; k < 3; k++){
                mat.at<cv::Vec3b>(i, j)[k] = inputImage[i][j][k];
            }
        }
    }
...
double ColorImage::calculatePSNR(std::vector<std::vector<std::vector<unsigned char> > > compareImage, int channel) {
