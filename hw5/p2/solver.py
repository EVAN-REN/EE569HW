...
class Solver(object):
    ...
    def test(self, train=True):
        train_acc,_ = self.test_dataset(self.train_loader)
        print(f"Train acc: {train_acc:.2%}\nTrain Confusion Matrix:")
        # print(cm)

        test_acc,_ = self.test_dataset(self.test_loader)
        print(f"Test acc: {test_acc:.2%}\nTest Confusion Matrix:")
        # print(cm)

        return train_acc,test_acc
    
    def display(self,train_acc1, test_acc1):
        plt.plot(range(1, len(train_acc1) + 1), train_acc1, label='trainAcc lr=0.001 dc=0.00001')
        plt.plot(range(1, len(test_acc1) + 1), test_acc1, label='test_acc lr=0.001 dc=0.00001')
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            train_acc,test_acc = self.test() # Test training set every 25 epochs
            ...
            if epoch < self.args.warmup_epochs:
                ...
                cos_decay.step()
        print("image show :")
        self.display(train_acc1,test_acc1)
